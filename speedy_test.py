...
def listSplit():
    ...
    y = 1
    positioner_up_one = 0
    amount_of_seperators = 4*(30)
    #multiple
    #m = 3 #how long the string would be
    #positioner = range(n, (m * n)+1, n)
    #print positioner
    while positioner_up_one <= (amount_of_seperators * 7) - 1:
        step_compensator += 0
        #print step_compensator
        positioner_up_one += 44 + step_compensator
...
#        print mini_test
        #list.remove(item)
        #return splitList
        #micro_multipier = [x for x in mini_test if x >= 0 ]
        #print micro_multipier
    def sublist(somelist):
        split = []
        r = []
        for i in somelist:
...
def multipier():
    multer = split #[[5165,5165,5165,5165,5165,5165,5165]]
    sub_names= sub
    output = []
